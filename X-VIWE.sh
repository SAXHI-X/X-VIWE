...
show_menu() {
    echo "Welcome to X-VIWE Installer/Uninstaller"
    echo "Please choose an option:"
    echo "1. Run X-VIWE (Install)"
    echo "2. Uninstall X-VIWE"
    echo "3. Exit"
}
...
while true; do
    ...
    case $CHOICE in
        1)
            echo "Proceeding with X-VIWE installation..."
            break
            ;;
        2)
...
cd /home/$USERNAME
if curl -L "$DOWNLOAD_URL" -o X-VIWE.zip; then
    echo "Download successful. Extracting files..."
    unzip -o X-VIWE.zip -d /home/$USERNAME
    EXTRACTED_DIR=$(ls /home/$USERNAME | grep "X-VIWE-" | head -n 1)
    mv "/home/$USERNAME/$EXTRACTED_DIR" /home/$USERNAME/X-VIWE
    rm X-VIWE.zip
else
    echo "Failed to download X-VIWE version $VERSION. Exiting."
    exit 1
fi

# Verify the templates directory exists
if [ -d "/home/$USERNAME/X-VIWE/templates" ]; then
...
mkdir -p /var/lib/X-VIWE/certs
sudo chown -R $USERNAME:$USERNAME /var/lib/X-VIWE/certs

# Check if valid certificate already exists
if [ -f "/var/lib/X-VIWE/certs/$DOMAIN.cer" ] && [ -f "/var/lib/X-VIWE/certs/$DOMAIN.cer.key" ]; then
    echo "Valid SSL certificate already exists."
    SSL_CONTEXT="--certfile=/var/lib/X-VIWE/certs/$DOMAIN.cer --keyfile=/var/lib/X-VIWE/certs/$DOMAIN.cer.key"
else
    echo "Generating SSL certificate..."
    curl https://get.acme.sh | sh -s email=$USERNAME@$SERVER_IP
    ~/.acme.sh/acme.sh --issue --force --standalone -d "$DOMAIN" \
        --fullchain-file "/var/lib/X-VIWE/certs/$DOMAIN.cer" \
        --key-file "/var/lib/X-VIWE/certs/$DOMAIN.cer.key"
    # Fix ownership of the generated certificates
    sudo chown $USERNAME:$USERNAME /var/lib/X-VIWE/certs/$DOMAIN.cer
    sudo chown $USERNAME:$USERNAME /var/lib/X-VIWE/certs/$DOMAIN.cer.key
...
if sudo systemctl is-active --quiet X-VIWE; then
    echo "Stopping existing X-VIWE service..."
    sudo systemctl stop X-VIWE
fi

# Create a systemd service to keep the Flask app running with Gunicorn
echo "Setting up systemd service..."
cat > /etc/systemd/system/X-VIWE.service <<EOL
[Unit]
Description=X-VIWE Web App
After=network.target

[Service]
User=$USERNAME
WorkingDirectory=/home/$USERNAME/X-VIWE
ExecStart=/bin/bash -c 'source /home/$USERNAME/X-VIWE/venv/bin/activate && exec gunicorn -w 4 -b 0.0.0.0:$PORT $SSL_CONTEXT app:app'
Environment="DB_PATH=/etc/x-ui/x-ui.db"
Restart=always
RestartSec=5
StandardOutput=append:/var/log/X-VIWE.log
StandardError=append:/var/log/X-VIWE.log
SyslogIdentifier=X-VIWE
...
sudo systemctl enable X-VIWE
sudo systemctl start X-VIWE
